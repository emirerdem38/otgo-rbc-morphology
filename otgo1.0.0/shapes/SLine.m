classdef SLine < Shape
    ...
    methods
        ...
        function h = plot(ln,varargin)
            ...
            %
            % H = plot(LN,'ColorAlpha',A) sets the line opacity to A in [0,1].
            %   Default A = 0.22.
            %
            ...
            % Default line appearance (semi-transparent orange rays)
            lineColor = [1, 0.55, 0.20];
            lineAlpha = 0.22;
            lineWidth = 1;
            for n = 1:2:length(varargin)
                if strcmpi(varargin{n}, 'ColorAlpha')
                    lineAlpha = varargin{n+1};
                    Check.isreal('ColorAlpha must be a real number in [0,1]', lineAlpha, '>=', 0);
                    Check.isreal('ColorAlpha must be a real number in [0,1]', lineAlpha, '<=', 1);
                ...
                elseif strcmpi(varargin{n}, 'LineWidth')
            ...
            % ht = plot3(S*X,S*Y,S*Z,'yellow');
            % ht = plot3(S*X, S*Y, S*Z, 'Color', [1, 0.5, 0, 0.15], 'LineWidth', 1);
            ht = plot3(S*X, S*Y, S*Z, ...
            ...
            % Sets properties
            skipProps = {'range', 'scale', 'coloralpha', 'color', 'linewidth'};
            for n = 1:2:length(varargin)
                if any(strcmpi(varargin{n}, skipProps))
                    continue;
                end
        ...
        function [rho, u, v] = topolar(ln, numDigits)
            % TOPOLAR Line to symbollic polar expression
            %
            % [RHO,U,V] = TOPOLAR(LN) calculates the smybolic expressions
            %   by converting the cartesian components of the line to polar
            %   coordinates.
            %
            % See also SLine.
            ...
            syms t
            lnc = ln.p2 - ln.p1;
            rho = sqrt((ln.p1.X+t*lnc.X)^2+(ln.p1.Y+t*lnc.Y)^2+(ln.p1.Z+t*lnc.Z)^2);
            u = acos((ln.p1.Z+t*lnc.Z)/rho);
            v = atan2(ln.p1.Y+t*lnc.Y,ln.p1.X+t*lnc.X);

            % Apply vpa to limit the precision to numDigits digits
