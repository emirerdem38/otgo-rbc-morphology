classdef CSVCellArbitrary < Superficies
    % CSVCellArbitrary < Superficies : Numeric cell loaded from a csv file.
    ...
    %
    % CSVCellArbitrary methods:
    %   CSVCellArbitrary    -   constructor
    ...
    methods
        function obj = CSVCellArbitrary(c,or1,or2,or3,cart,mag)
            % CSVCELLArbitrary(c,or1,or2,or3,cartvol,mag) constructs a Cell
            ...
            shp = alphaShape(cart.x,cart.y,cart.z,0.3);
            shp = alphaShape([cart.x,cart.y,cart.z],criticalAlpha(shp,'one-region') + 0.1);
