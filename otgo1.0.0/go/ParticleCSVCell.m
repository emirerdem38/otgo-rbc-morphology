classdef ParticleCSVCell < Particle
    % ParticleCSVCELL < Particle : Cell optically trappable particle
    ...
    methods
        function obj = ParticleCSVCell(c,or1,or2,or3,vol,mag,data,nm,np)
            % PARTICLECSVCELL(c,or1,or2,or3,mag,vol,nm,np) constructs a CELL particle
            %   with center c, orientations or1, or2, or3, volume vol, order of magnitude mag,
            %   loading address of cell data, medium refractive index nm and particle refractive index np.
            %   c must be a single Point, or1, or2 and or3 must be single vectors, mag must be integer,
            %   nm and np must be real numbers.
            %
            % See also CSVCell, Point.
            ...
            obj.cell = CSVCell(c,or1,or2,or3,vol,mag,data);
            obj.nm = nm;
            obj.np = np;
        end
        ...
        function r_vec = scattering(particle,r,err,N,varargin)
            % SCATTERING Scattered rays
            %
