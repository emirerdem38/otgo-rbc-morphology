classdef ParticleEllipsoidal < Particle
    ...
    methods
        ...
        function f = force(particle,r_vec,r)
            ...
            f = fi-fe;
            f.X = particle.elli.c.X.*ones(size(f));
            f.Y = particle.elli.c.Y.*ones(size(f));
            f.Z = particle.elli.c.Z.*ones(size(f));
        end
        function T = torque(particle,r_vec,r)
            ...
            T = Ti - Te;
            T.X = particle.elli.c.X.*ones(size(T));
            T.Y = particle.elli.c.Y.*ones(size(T));
            T.Z = particle.elli.c.Z.*ones(size(T));
        end
    end
end